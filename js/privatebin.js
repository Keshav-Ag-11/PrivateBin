...
window.PrivateBin = (function () {
    ...
    const CryptTool = (function () {
        ...
        me.cipher = async function (key, password, message, adata) {
            ...
            // authentication tag 128 bit, Argon2id key derivation
            const compression = (
                ...
                spec = [
                    ...
                    100000,             // iterations (legacy value, unused by argon2id pastes)
                    256,                // key size
                    128,                // tag size
                    'aes',              // algorithm
                    'gcm',              // algorithm mode
        ...
        // --- DECOY / DENIABLE ENCRYPTION ENGINE ---
        const PADDING_BUCKETS = [1024, 4096, 16384, 65536, 262144, 1048576];

        function selectBucket(byteLength) {
            const bucket = PADDING_BUCKETS.find((b) => b >= byteLength + 4);
            if (bucket === undefined) {
                throw new Error('Content exceeds maximum supported paste size (1MB)');
            }
            return bucket;
        }
        ...
        me.selectBucket = selectBucket;
        me.padToLength = padToLength;
        me.unpad = unpad;

        async function deriveDuressLayerKey(masterKey, password, saltBytes) {
            const secret = masterKey + (password && password.length > 0 ? password : '');
            const derivedKey = await hashwasm.argon2id({
                password: secret,
                salt: saltBytes,
                parallelism: 1,
        ...
        async function encryptDuressLayer(paddedPlaintext, password, masterKey) {
            const salt = window.crypto.getRandomValues(new Uint8Array(16));
            const nonce = window.crypto.getRandomValues(new Uint8Array(12));
            const key = await deriveDuressLayerKey(masterKey, password, salt);
            ...
            return {
                ciphertext: btoa(arraybufferToString(ciphertextBuffer)),
                nonce: btoa(arraybufferToString(nonce)),
                salt: btoa(arraybufferToString(salt)),
                kdfParams: { algorithm: 'argon2id', iterations: 3, memorySize: 19456, parallelism: 1 }
            };
        }

        me.encryptDuressPaste = async function (contentA, passwordA, contentB, passwordB, masterKey) {
            const bytesA = new TextEncoder().encode(contentA);
            const bytesB = new TextEncoder().encode(contentB);

            const bucket = selectBucket(Math.max(bytesA.length, bytesB.length));

            const [layerA, layerB] = await Promise.all([
                encryptDuressLayer(padToLength(bytesA, bucket), passwordA, masterKey),
                encryptDuressLayer(padToLength(bytesB, bucket), passwordB, masterKey)
            ]);
        ...
        me.encryptSinglePasswordDuressPaste = async function (content, password, masterKey) {
            const discardedPassword = btoa(arraybufferToString(window.crypto.getRandomValues(new Uint8Array(32))));
            return me.encryptDuressPaste(content, password, '', discardedPassword, masterKey);
        };

        async function tryDecryptDuressLayer(layer, password, masterKey) {
            try {
                const salt = stringToArraybuffer(atob(layer.salt));
                const nonce = stringToArraybuffer(atob(layer.nonce));
                const ciphertext = stringToArraybuffer(atob(layer.ciphertext));
                ...
                return new TextDecoder().decode(unpad(new Uint8Array(plaintextBuffer)));
            } catch (e) {
                return null;
            }
        }

        me.decryptDuressBlob = async function (blob, password, masterKey) {
            const [layerA, layerB] = blob.layers;

            const [resultA, resultB] = await Promise.all([
                tryDecryptDuressLayer(layerA, password, masterKey),
                tryDecryptDuressLayer(layerB, password, masterKey)
            ]);
        ...
        /**
         * generate an ECDH P-256 keypair for a recipient
         *
         * Returns { privateKeyBytes, publicKeyBytes } as raw Uint8Array buffers.
         * The private key is embedded in the recipient's personal URL fragment.
         * The public key is stored on the server (inside the paste envelope).
         *
         * @name   CryptTool.generateRecipientKeypair
         * @async
         * @function
         * @return {Promise<{privateKeyBytes: Uint8Array, publicKeyBytes: Uint8Array}>}
         */
        me.generateRecipientKeypair = async function () {
            const keypair = await window.crypto.subtle.generateKey(
                { name: 'ECDH', namedCurve: 'P-256' },
                true,  // extractable so we can embed private key in URL
                ['deriveKey', 'deriveBits']
            );
            const privateKeyJwk = await window.crypto.subtle.exportKey('jwk', keypair.privateKey);
            const publicKeyJwk  = await window.crypto.subtle.exportKey('jwk', keypair.publicKey);

            // encode both as compact JSON strings, then to UTF-8 bytes for base58 transport
            const privateKeyBytes = stringToArraybuffer(JSON.stringify(privateKeyJwk));
            const publicKeyBytes  = stringToArraybuffer(JSON.stringify(publicKeyJwk));
            return { privateKeyBytes, publicKeyBytes };
        };

        /**
         * wrap a CEK for one recipient using ECDH P-256 + AES-KW
         *
         * Algorithm (inspired by the `age` tool):
         *   1. Generate a fresh ephemeral ECDH keypair.
         *   2. Derive a 256-bit shared secret: ECDH(ephemeralPriv, recipientPub).
         *   3. AES-KW wrap the CEK using that shared secret.
         *   4. Return the wrapped CEK bytes + the ephemeral public key bytes
         *      (both as base64 strings ready for server storage).
         *
         * @name   CryptTool.wrapKeyForRecipient
         * @async
         * @function
         * @param  {string} cekBytes        raw CEK bytes (string / output of getSymmetricKey)
         * @param  {string} recipientPubJwk JSON string of recipient public key JWK
         * @return {Promise<{wrappedKey: string, ephemeralPublicKey: string}>}  base64 strings
         */
        me.wrapKeyForRecipient = async function (cekBytes, recipientPubJwk) {
            // import recipient's public key
            const recipientPublicKey = await window.crypto.subtle.importKey(
                'jwk',
                JSON.parse(recipientPubJwk),
                { name: 'ECDH', namedCurve: 'P-256' },
                false,
                []
            );
            ...
            // import the raw CEK as an AES-GCM key so WebCrypto can wrap it
            const cekKey = await window.crypto.subtle.importKey(
                'raw',
                stringToArraybuffer(cekBytes),
                { name: 'AES-GCM' },
                true,
                ['encrypt', 'decrypt']
            );
            ...
            // export the ephemeral public key as JWK JSON
            const ephemeralPublicKeyJwk = await window.crypto.subtle.exportKey('jwk', ephemeralKeypair.publicKey);

            return {
                wrappedKey:        btoa(arraybufferToString(wrappedKeyBuffer)),
                ephemeralPublicKey: btoa(JSON.stringify(ephemeralPublicKeyJwk))
            };
        };

        /**
         * unwrap a CEK for a recipient using ECDH P-256 + AES-KW
         *
         * Reverses wrapKeyForRecipient:
         *   1. Import recipient private key + ephemeral public key.
         *   2. Derive shared secret: ECDH(recipientPriv, ephemeralPub).
         *   3. AES-KW unwrap the wrapped CEK.
         *   4. Export the raw CEK bytes (ready for use in CryptTool.decipher).
         *
         * @name   CryptTool.unwrapKeyForRecipient
         * @async
         * @function
         * @param  {string} wrappedKeyB64        base64 wrapped CEK
         * @param  {string} ephemeralPublicKeyB64 base64 JSON of ephemeral public key JWK
         * @param  {string} recipientPrivKeyB64   base64 JSON of recipient private key JWK
         * @return {Promise<string>} raw CEK bytes (same format as getSymmetricKey output)
         */
        me.unwrapKeyForRecipient = async function (wrappedKeyB64, ephemeralPublicKeyB64, recipientPrivKeyB64) {
            // import ephemeral public key
            const ephemeralPublicKeyJwk = JSON.parse(atob(ephemeralPublicKeyB64));
            ...
            const recipientPrivKeyJwk = JSON.parse(atob(recipientPrivKeyB64));
            ...
            // AES-KW unwrap to get the raw CEK as an AES-GCM CryptoKey
            const wrappedKeyBytes = stringToArraybuffer(atob(wrappedKeyB64));
            const cekKey = await window.crypto.subtle.unwrapKey(
                'raw',
                wrappedKeyBytes,
                unwrappingKey,
                { name: 'AES-KW' },
            ...
            // export the raw CEK bytes as a string (same format as getSymmetricKey)
            const rawCek = await window.crypto.subtle.exportKey('raw', cekKey);
            return arraybufferToString(rawCek);
        };
    ...
    const TopNav = (function (window, document) {
        ...
        let createButtonsDisplayed = false,
            ...
            retryButtonCallback,
            // envelope encryption (multi-recipient) UI state
            recipientsSection,
            recipientToggle,
            recipientList,
            recipientRowCount = 0;
        ...
        /**
         * Add a recipient input row to the recipient list.
         *
         * @name   TopNav.addRecipientRow
         * @private
         * @function
         */
        function addRecipientRow() {
            ...
            row.id = 'recipientrow-' + rowId;
            row.innerHTML =
                '<input id="recipientname-' + rowId + '" type="text" class="form-control form-control-sm recipient-name-input" ' +
                    'placeholder="Recipient name…" aria-label="Recipient name" />' +
                '<button type="button" class="btn btn-outline-danger btn-sm remove-recipient" data-rowid="' + rowId + '" ' +
                    'aria-label="Remove recipient">' +
                    '<svg width="14" height="14" fill="currentColor" aria-hidden="true"><use href="img/bootstrap-icons.svg#x" /></svg>' +
                '</button>';
            row.querySelector('.remove-recipient').addEventListener('click', function () {
                const rid = this.getAttribute('data-rowid');
                const el = document.getElementById('recipientrow-' + rid);
                if (el) {
                    el.remove();
                }
            });
        ...
        /**
         * Toggle multi-recipient section visibility and hide/show the password field.
         *
         * @name   TopNav.toggleRecipientsSection
         * @private
         * @function
         */
        function toggleRecipientsSection() {
            if (!recipientsSection) {
                return;
            }
            const isVisible = !recipientsSection.classList.contains('hidden');
            if (isVisible) {
                ...
                // hide the traditional password field (incompatible with envelope mode)
                if (password) {
                    password.classList.add('hidden');
                }
                // auto-add first row if list is empty
        ...
        me.hideCreateButtons = function () {
            ...
            // hide recipients toggle + collapse recipients section
            if (recipientToggle) {
                recipientToggle.classList.add('hidden');
            }
            if (recipientsSection && !recipientsSection.classList.contains('hidden')) {
                recipientsSection.classList.add('hidden');
            }
         ...
         * Returns an empty array if the recipients section is hidden (mode not active).
         *
         * @name   TopNav.getRecipients
         * @function
         * @return {Array<{name: string}>}
         */
        me.getRecipients = function () {
            if (!recipientsSection || recipientsSection.classList.contains('hidden')) {
                return [];
            }
            const inputs = recipientList ? recipientList.querySelectorAll('.recipient-name-input') : [];
            const result = [];
            inputs.forEach(function (input, index) {
                const name = input.value.trim() || ('Recipient ' + (index + 1));
                result.push({ name });
            });
            return result;
        };
        ...
        function bindEvents() {
            ...
            // multi-recipient envelope UI
            const toggleRecipientsBtn = document.getElementById('togglerecipients');
            if (toggleRecipientsBtn) {
                toggleRecipientsBtn.addEventListener('click', toggleRecipientsSection);
            }
        ...
        me.init = function () {
            ...
            // envelope encryption UI
            recipientsSection  = document.getElementById('recipientssection');
            recipientToggle    = document.getElementById('recipientstoggle');
    ...
    const ServerInteraction = (function () {
        ...
        function success(status, result) {
            if (successFunc !== null) {
                ...
                // pass through per-recipient private keys for envelope mode
                if (recipientPrivateKeys !== null) {
                    result.recipientPrivateKeys = recipientPrivateKeys;
                }
        ...
        me.prepare = function () {
            ...
            // reset per-recipient private keys
            recipientPrivateKeys = null;
        ...
        /**
         * returns the current symmetric encryption key (CEK)
         *
         * Used by PasteEncrypter in envelope mode to retrieve the CEK after
         * setCipherMessage() has generated it, so it can be wrapped per-recipient.
         *
         * @name   ServerInteraction.getEncryptionKey
         * @function
         * @return {string} raw CEK bytes
         */
        me.getEncryptionKey = function () {
            return symmetricKey;
        };

        /**
         * stash per-recipient private key blobs so the success callback can build
         * per-recipient URLs without the server ever seeing them.
         *
         * @name   ServerInteraction.setRecipientPrivateKeys
         * @function
         * @param {Array<{name: string, privateKeyB64: string}>} keys
         */
        me.setRecipientPrivateKeys = function (keys) {
    ...
    const PasteEncrypter = (function () {
        ...
        function showCreatedPaste(status, data) {
            ...
            // --- envelope (multi-recipient) mode ---
            if (data.recipientPrivateKeys && data.recipientPrivateKeys.length > 0) {
                // In envelope mode the "canonical" URL uses the first recipient's key.
                // The browser bar reflects that first recipient only.
                const firstPrivKey = data.recipientPrivateKeys[0].privateKeyB58;
                const url = baseUri + data.id + '#' + firstPrivKey;
                const deleteUrl = baseUri + 'pasteid=' + data.id + '&deletetoken=' + data.deletetoken;

                PasteStatus.createPasteNotification(url, deleteUrl);
                history.pushState({ type: 'newpaste' }, document.title, url);

                // Show per-recipient link cards
                const recipientLinksSection = document.getElementById('recipientlinks');
                const recipientLinkContainer = document.getElementById('recipientlinkcontainer');
                const linkTemplate = document.getElementById('recipientlinktemplate');
                if (recipientLinksSection && recipientLinkContainer && linkTemplate) {
                    recipientLinkContainer.innerHTML = '';
                    data.recipientPrivateKeys.forEach(function (recipientInfo) {
                        const recipientUrl = baseUri + data.id + '#' + recipientInfo.privateKeyB58;
                        const card = linkTemplate.cloneNode(true);
                        card.id = '';
                        card.classList.remove('hidden');
                        card.querySelector('.recipient-link-name').textContent = recipientInfo.name;
                        const urlInput = card.querySelector('.recipient-link-url');
                        urlInput.value = recipientUrl;
                        card.querySelector('.recipient-link-copy').addEventListener('click', function () {
                            navigator.clipboard.writeText(recipientUrl).catch(function () {
                                urlInput.select();
                                document.execCommand('copy');
                            });
                        });
                        recipientLinkContainer.appendChild(card);
                    });
                    recipientLinksSection.classList.remove('hidden');
                }

                // copy button targets first recipient URL
                CopyToClipboard.setUrl(url);
                CopyToClipboard.showKeyboardShortcutHint();
            } else {
                // --- standard (single-key) mode ---
                const url = baseUri + data.id + (TopNav.getBurnAfterReading() ? loadConfirmPrefix : '#') + CryptTool.base58encode(data.encryptionKey);
                const deleteUrl = baseUri + 'pasteid=' + data.id + '&deletetoken=' + data.deletetoken;
                PasteStatus.createPasteNotification(url, deleteUrl);
                history.pushState({ type: 'newpaste' }, document.title, url);
                CopyToClipboard.setUrl(url);
    ...
    // Get recipients BEFORE hiding the recipient section
    const recipientList = TopNav.getRecipients();
    const envelopeMode = recipientList.length > 0;
    ...
    // in envelope mode we do NOT use a user-typed password
    ServerInteraction.setCryptParameters(
        envelopeMode ? '' : TopNav.getPassword()
            ...
            // encrypt message (generates or uses the symmetricKey inside ServerInteraction)
            await ServerInteraction.setCipherMessage(cipherMessage).catch(Alert.showError);

            // --- envelope (multi-recipient) mode: wrap CEK per recipient ---
            if (envelopeMode) {
                Alert.showLoading('Generating recipient keys…', 'cloud-upload');
                // retrieve the CEK that setCipherMessage just generated
                const cek = ServerInteraction.getEncryptionKey();
                const recipientEnvelopes = [];
                const recipientPrivateKeys = [];

                for (const recipient of recipientList) {
                    // 1. generate a fresh ECDH P-256 keypair for this recipient
                    const keypair = await CryptTool.generateRecipientKeypair();
                    const publicKeyB64  = btoa(String.fromCharCode(...new Uint8Array(keypair.publicKeyBytes)));
                    // private key embedded in the recipient's personal URL fragment (base64)
                    const privateKeyB58 = CryptTool.base58encode(
    String.fromCharCode(...new Uint8Array(keypair.privateKeyBytes))
);
                    // 2. wrap the CEK for this recipient
                    // publicKeyBytes contains UTF-8 bytes of the JWK JSON string; decode back to string
                    const recipientPubJwk = new TextDecoder().decode(keypair.publicKeyBytes);
                    const wrapped = await CryptTool.wrapKeyForRecipient(
                        cek,
                        recipientPubJwk
                    );
                    ...
                    recipientPrivateKeys.push({
                        name:          recipient.name,
                        privateKeyB58: privateKeyB58
                    });
                }

                // 3. embed envelopes in meta (stored on server — zero raw key material)
                ServerInteraction.setUnencryptedData('meta', {
                    'expire':     TopNav.getExpiration(),
                    'recipients': recipientEnvelopes
                });

                // 4. stash private keys so showCreatedPaste can build per-recipient URLs
                ServerInteraction.setRecipientPrivateKeys(recipientPrivateKeys);
                Alert.showLoading('Sending document…', 'cloud-upload');
            }
    ...
    const PasteDecrypter = (function () {
        ...
        me.run = function (paste) {
            ...
            // --- envelope (multi-recipient) mode detection ---
            const recipients = (paste.meta && paste.meta.recipients) ? paste.meta.recipients : null;
            if (recipients && recipients.length > 0) {
                // The URL fragment holds the recipient's base64-encoded private key JWK JSON
                const rawFragment = window.location.hash.substring(1);
let recipientPrivKeyB58 = rawFragment;

// Strip any trailing &utm_* params etc.
const amp = recipientPrivKeyB58.indexOf('&');
if (amp > -1) {
    recipientPrivKeyB58 = recipientPrivKeyB58.substring(0, amp);
}

// Convert the Base58 URL key back to the original private-key JSON,
// because unwrapKeyForRecipient() expects Base64.
const recipientPrivKeyB64 = btoa(
    CryptTool.base58decode(recipientPrivKeyB58)
);
                // Attempt to unwrap with each envelope entry until one succeeds.
                // This is necessary because we don't store a key-ID in the URL.
                (async () => {
                    let cek = null;
                    let lastErr = null;
                    for (const envelope of recipients) {
                        try {
                            cek = await CryptTool.unwrapKeyForRecipient(
                                envelope.wrappedKey,
                    ...
                    if (!cek) {
                        console.error('Envelope unwrap failed:', lastErr);
                        Alert.hideLoading();
                        Alert.showError('Could not decrypt: your link does not match any recipient key in this document.');
                        return;
                    }

                    // use unwrapped CEK as the decryption key (no password)
                    const password = Prompt.getPassword();
                    const decryptionPromises = [decryptPaste(paste, cek, password)];
                    if (paste.isDiscussionEnabled()) {
                        decryptionPromises.push(decryptComments(paste, cek, password));
                    }
                    ...
                    Promise.all(decryptionPromises)
                        .then(() => {
                            ...
                            if (paste.isBurnAfterReadingEnabled()) {
                                ...
                                TopNav.showEmailButton(paste.getTimeToLive(), paste.isBurnAfterReadingEnabled());
                            }
                            if (paste.isDiscussionEnabled()) {
                ...
                return; // the async IIFE above handles the rest
            }
